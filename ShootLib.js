...
/*  ChangeLog:
    ...
    v1.4
        ...
        - Fix GUI Fov
		- Fix shotgun shooting, now the damage is fully applied.
		- Minor fixs
        - Upgrade SoundLib to 2.3
    v1.3
        ...
        for(var i in settings){
            switch(i){
                ...
                case "aim":
                    ...
                    if(!text)
                        text = {content:i.toUpperCase(), size:18}
                    if(!text.content)
                        text.content = i.toUpperCase();
                    if(!text.size)
                        text.size = 18;
...
Callback.addCallback("ProjectileHit", function (p,i,t) {
    if(bullets.hasOwnProperty(p)){
        if(t.entity == -1){
            ...
            if(!entitys_hurt[t.entity]) entitys_hurt[t.entity] = [];
